...
class TestChecks:
    @pytest.mark.parametrize(
        "check", (*EVERY_CHECK, *SECOND_WAVE), ids=lambda c: c.__name__
    )
    ...
    def test_every_check_carries_a_promise(self):
        for check in (*EVERY_CHECK, *SECOND_WAVE):
            result = check()
...
class TestSuite:
    def test_run_covers_every_check(self):
        suite = Conformance()
        assert len(suite.run()) == len(EVERY_CHECK) + len(SECOND_WAVE)
    ...
    def test_the_report_reads_pass_by_pass(self):
        ...
        assert report.count("[pass]") == len(EVERY_CHECK) + len(SECOND_WAVE)
